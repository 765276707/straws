...
export default {
    ...
    methods: {
        ...
        async getPage() {
            this.loading = true
            let response = await this.$API.user.getPage(this.searchText)   
        ...
        resetPassword(index, row) {
            this.$confirm('此操作将会重置该用户的用户密码，仅限超级管理员进行操作，是否继续?', '危险操作', {
                ...
                this.$prompt('请输入您的密码，以确认重置', '确认重置', {
        ...
        deleteInRow(index, row) {
            this.$confirm('此操作将会删除该用户，仅限超级管理员进行操作，是否继续?', '危险操作', {
                confirmButtonText: '继续',
                cancelButtonText: '取消',
                type: 'warning',
                center: true
